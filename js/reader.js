...
// V73 is authoritative when it contains the tap. V81 fallback runs only after a miss.
// V81 is based on V79; only fallback candidate selection changes.
...
const Reader = {
 ...
 async loadPanelsForCurrentPage() {
   ...
   const logger = this.debugMode ? (msg) => this.debugLog(`[V81 panels p${pageIndex}] ${msg}`) : null;
   ...
   if (logger) logger(`V81 currentPanels set: ${panels.length} fresh stable-gutter panel(s)`);
 },
...
async setMode(mode) {
 ...
 async handleSingleTap(pos) {
   ...
   if (panel) {
     if (this.debugMode) this.debugLog("[V81] PASS 1 HIT (V73 baseline)");
     this.zoomToPanel(panel, stageRect, imgRect);
     return;
   }

   // PASS 2: only the exact V73-missed tap gets the new local geometry test.
   if (this.debugMode) this.debugLog("[V81] PASS 1 MISS -> tap-local geometry fallback");
   const pageIndex = this.index;
   const comicId = this.comic?.id;
   const url = await this.getPageUrl(pageIndex);
   if (url && PanelDetect.detectTapLocalFallback) {
     const logger = this.debugMode
       ? (msg) => this.debugLog(`[V81 fallback p${pageIndex}] ${msg}`)
       : null;
     ...
     if (this.comic?.id === comicId && this.index === pageIndex && fallback) {
       if (this.debugMode) this.debugLog("[V81] PASS 2 HIT -> zoom fallback panel");
       this.zoomToPanel(fallback, stageRect, imgRect);
       return;
     }
     if (this.debugMode) this.debugLog("[V81] PASS 2 MISS");
   }
