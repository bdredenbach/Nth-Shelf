...
// V73 is authoritative when it contains the tap. V80 fallback runs only after a miss.
// V80 is based on V78; only fallback acceptance is loosened.
...
const Reader = {
 ...
 async loadPanelsForCurrentPage() {
   ...
   const logger = this.debugMode ? (msg) => this.debugLog(`[V80 panels p${pageIndex}] ${msg}`) : null;
   ...
   if (logger) logger(`V80 currentPanels set: ${panels.length} fresh stable-gutter panel(s)`);
 },
...
async setMode(mode) {
 ...
 async handleSingleTap(pos) {
   ...
   if (panel) {
     if (this.debugMode) this.debugLog("[V80] PASS 1 HIT (V73 baseline)");
     this.zoomToPanel(panel, stageRect, imgRect);
     return;
   }

   // PASS 2: only the exact V73-missed tap gets the new local geometry test.
   if (this.debugMode) this.debugLog("[V80] PASS 1 MISS -> tap-local geometry fallback");
   const pageIndex = this.index;
   const comicId = this.comic?.id;
   const url = await this.getPageUrl(pageIndex);
   if (url && PanelDetect.detectTapLocalFallback) {
     const logger = this.debugMode
       ? (msg) => this.debugLog(`[V80 fallback p${pageIndex}] ${msg}`)
       : null;
     ...
     if (this.comic?.id === comicId && this.index === pageIndex && fallback) {
       if (this.debugMode) this.debugLog("[V80] PASS 2 HIT -> zoom fallback panel");
       this.zoomToPanel(fallback, stageRect, imgRect);
       return;
     }
     if (this.debugMode) this.debugLog("[V80] PASS 2 MISS");
   }
