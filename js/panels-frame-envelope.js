// NTH SHELF V2.78.14 — CONNECTED FRAME / BOUNDARY WALK
//
// Recover one connected four-sided frame around the tapped panel.  Unlike
// V2.78.13, corners are NEVER searched or snapped independently.  We first
// prove four continuous outer rails; the only legal vertices are the
// intersections of those same neighboring rails.  Each intersection must be
// supported by BOTH rails close to the meeting point.
...
const PanelFrameEnvelope = {
    ...
    return new Promise((resolve) => {
      ...
      img.onload = () => {
        try { resolve(this._detect(img, panel, log)); }
          ...
          if (log) log(`BOUNDARY WALK ERROR ${err.message}`);
          resolve(null);
        }
      };
  ...
  _detect(img, panel, log) {
    ...
    if(log)log(`BOUNDARY WALK start tap=${tx.toFixed(1)},${ty.toFixed(1)} seed=${x0},${y0}-${x1},${y1}`);
    ...
    const fitRail=(kind)=>{
      ...
      const evaluate=(m,anchor)=>{
        ...
        return {kind,horizontal,m,b,anchor,atTap,support,continuity,strongRate,segments,score};
      };
      ...
      if(log)log(best?`BOUNDARY WALK rail ${kind} HIT m=${best.m.toFixed(3)} support=${best.support.toFixed(2)} cont=${best.continuity.toFixed(2)} atTap=${best.atTap.toFixed(1)}`:`BOUNDARY WALK rail ${kind} MISS`);
      return best;
    };

    const top=fitRail('top'),bottom=fitRail('bottom'),left=fitRail('left'),right=fitRail('right');
    const rails=[top,bottom,left,right];
    if(rails.some(r=>!r)){if(log)log(`BOUNDARY WALK MISS rails=${rails.filter(Boolean).length}/4`);return null;}
    ...
    if(q.some(p=>!p||!Number.isFinite(p.x)||!Number.isFinite(p.y))){if(log)log('BOUNDARY WALK MISS intersections');return null;}
    ...
    // vertex itself is never moved. This is the core V2.78.14 change.
    ...
    for(let i=0;i<4;i++){
      const p=q[i];
      if(p.x<-8||p.x>w+8||p.y<-8||p.y>h+8){if(log)log(`BOUNDARY WALK MISS corner-${i} outside`);return null;}
      const a=armSupport(p,pairs[i][0],pairs[i][1]);
      if(log)log(`BOUNDARY WALK corner ${i} x=${p.x.toFixed(1)} y=${p.y.toFixed(1)} arms=${a.h.toFixed(2)}/${a.v.toFixed(2)}`);
      if(a.h<.28||a.v<.28){if(log)log(`BOUNDARY WALK MISS corner-${i} disconnected`);return null;}
    }
    ...
    if(!(cs.every(v=>v>0)||cs.every(v=>v<0))){if(log)log('BOUNDARY WALK MISS non-convex');return null;}

    const area=Math.abs(q.reduce((s,p,i)=>{const n=q[(i+1)%4];return s+p.x*n.y-n.x*p.y;},0)/2);
    const areaRatio=area/(rw*rh);
    if(areaRatio<.60||areaRatio>1.85){if(log)log(`BOUNDARY WALK MISS whole-frame area=${areaRatio.toFixed(2)}`);return null;}
    ...
    if(!pointInPoly(tx,ty,q)){if(log)log('BOUNDARY WALK MISS tap outside');return null;}
    ...
    for(let i=0;i<4;i++){
      ...
      if(dx>tolX||dy>tolY){if(log)log(`BOUNDARY WALK MISS corner-${i} seed-drift=${dx.toFixed(1)}/${dy.toFixed(1)}`);return null;}
    }
    ...
    if(log)log(`BOUNDARY WALK HIT area=${areaRatio.toFixed(2)} confidence=${confidence.toFixed(2)} quad=${quad.map(p=>`${p.x.toFixed(3)},${p.y.toFixed(3)}`).join(' | ')}`);
