...
(async()=>{
  let count=0;
  const unchanged=[[.25,.10],[.20,.40],[.45,.65],[.45,.85]];
  async function checkOutside(){
    const expected=reader.currentPanels.filter(p=>!p._identitySource);
    for(const [x,y]of unchanged){
      const old=expected.find(p=>x>=p.x&&x<=p.x+p.w&&y>=p.y&&y<=p.y+p.h);
      assert(old,'unchanged legacy owner exists');captures=[];
      await reader.handleSingleTap({x:x*600,y:y*900});assert.equal(captures.length,1);
      assert.deepEqual(shape(captures[0]),shape(old),'outside refinement, preserve exact original crop');
    }
  }
  for(const page of [11]){
    const panels=await fresh.api.PanelDetect.detect(fresh.pages[page-1]);
    assert.equal(panels.length,{11:4}[page],`page${page}: independently identified frame count`);
    ...
    await checkOutside(); count+=unchanged.length;
  ...
  if(process.argv.includes('--skia')){
    ...
    for(const quality of ['low','medium','high']){
      ...
      assert.equal(reader.currentPanels.length,4,`${quality}: refined fortifications plus three unchanged legacy identities`);
      ...
      await checkOutside();probes+=unchanged.length;
    }
