...
(async () => {
  ...
  assert.equal(w.name,'nth-shelf-shell-2.79.35'); passed++;
  assert(w.cache.files.includes('./js/panels-curved-rims.js')); passed++;
  assert(w.cache.files.includes('./js/panels-matte-cells.js')); passed++;
