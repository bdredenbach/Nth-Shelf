...
const output=process.env.NTH_QA_OUTPUT||'/tmp/nth-page19';fs.mkdirSync(output,{recursive:true});
...
const labels=require('./frame-accuracy/queue-artwork-27919.json').frames;
...
const url=origin+'/comic-wolverine-1000/'+encodeURIComponent(files[18]);
if(process.env.NTH_BASELINE_URL){
 ...
 assert.deepEqual(sweep.filter(p=>p.changed).map(p=>p.page),[19],'only targeted page changes');
 fs.writeFileSync(path.join(output,'browser-sweep.json'),JSON.stringify(sweep,null,2));await old.close();
 console.log('74-page comparison passed; only page 19 changes.');
}
const panels=await page.evaluate(async url=>{ShelfGuide.finish();ShelfGuide.seen.single=true;await LongboxDB.putPage('p19',0,await(await fetch(url)).blob());await LongboxDB.addComic({id:'p19',title:'Page 19',pageCount:1,lastPage:0,bookmarks:[],addedAt:Date.now(),readMode:'single',theme:'dark'});await LongboxApp.openReader('p19');await Reader._panelDetection?.promise;window.captures=[];const zoom=Reader.zoomToPanel;Reader.zoomToPanel=function(p,...args){captures.push(JSON.parse(JSON.stringify(p)));return zoom.call(this,p,...args);};return Reader.currentPanels;},url);
console.log('Identities',panels.map(p=>[p._identitySource,p.x,p.y,p.w,p.h]));assert.equal(panels.length,3);
const touches=[];
...
assert.deepEqual(errors,[]);fs.writeFileSync(path.join(output,'page19-reader.json'),JSON.stringify({labels,panels,touches},null,2));console.log('30 real Chromium touch/rendered-overlay checks passed.');
