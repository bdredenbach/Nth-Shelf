...
(async () => {
 ...
 try {
  ...
  const comicBox=await page.evaluate(()=>{const r=Reader.turnPageMode.book.pageNode().querySelector('img').getBoundingClientRect();return {left:r.left,right:r.right,top:r.top,bottom:r.bottom};});
  ...
  assert.ok(Math.abs(topCue.right-comicBox.right)<=6&&Math.abs(topCue.top-comicBox.top)<=6,'Top corner cue must align to the comic upper-right corner');
  assert.ok(Math.abs(bottomCue.right-comicBox.right)<=6&&Math.abs(bottomCue.bottom-comicBox.bottom)<=6,'Bottom corner cue must align to the comic lower-right corner, not navigation');
