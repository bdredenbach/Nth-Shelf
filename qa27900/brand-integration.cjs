...
(async () => {
 ...
 try {
  ...
  const cornerCues=await page.locator('.guide-corner-cue').evaluateAll(nodes=>nodes.map(node=>{const r=node.getBoundingClientRect();return {className:node.className,left:r.left,right:r.right,top:r.top,bottom:r.bottom,width:r.width,height:r.height};}));
  const comicBox=await page.evaluate(()=>{const r=Reader.turnPageMode.book.pageNode().querySelector('img').getBoundingClientRect(),m=Reader.turnPageMode.constructor.cornerZoneMetrics(r.width,r.height);return {left:r.left,right:r.right,top:r.top,bottom:r.bottom,size:m.size,inset:m.inset};});
  assert.equal(cornerCues.length,2,'Corner tutorial must show both live right-corner grab zones');
  const topCue=cornerCues.find(c=>c.className.includes(' top')),bottomCue=cornerCues.find(c=>c.className.includes(' bottom'));
  assert.ok(topCue&&bottomCue,'Corner tutorial must distinguish top and bottom cues');
  assert.ok(comicBox.inset>=24,'Forward corner zones must be inset from the extreme page corners');
  assert.ok(Math.abs(topCue.right-comicBox.right)<=6&&Math.abs(topCue.top-(comicBox.top+comicBox.inset))<=6,'Top cue must match the lowered live upper-right zone');
  assert.ok(Math.abs(bottomCue.right-comicBox.right)<=6&&Math.abs(bottomCue.bottom-(comicBox.bottom-comicBox.inset))<=6,'Bottom cue must match the raised live lower-right zone');
