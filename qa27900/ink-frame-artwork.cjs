...
const labels=require('./frame-accuracy/queue-artwork-27918.json');
...
(async()=>{
  let count=0;
  for(const page of [7,8,17,58]){
    const panels=await fresh.api.PanelDetect.detect(fresh.pages[page-1]);
    assert.equal(panels.length,{7:4,8:3,17:6,58:1}[page],`page${page}: independently identified frame count`);
  ...
  if(process.argv.includes('--skia')){
    ...
    const closed=require('../js/panels-closed-frames'),partition=require('../js/panels-partition');
    let probes=0;
    for(const page of [7,17])for(const quality of ['low','medium','high']){
      const img=await loadImage(fresh.pages[page-1]),w=Math.round(img.width*900/img.height),h=900,c=createCanvas(w,h),ctx=c.getContext('2d');
      ctx.imageSmoothingQuality=quality;ctx.drawImage(img,0,0,w,h);const data=ctx.getImageData(0,0,w,h).data;
      const original=closed.analyzeRGBA(data,w,h),parts=partition.analyzeRGBA(data,w,h,null,{allowPartial:original.length>0,anchors:original});
      const base=fresh.api.PanelDetect._completePartition(original,parts);
      reader.currentPanels=base.concat(closed.analyzeRGBA(data,w,h,null,{supplementOnly:true,anchors:base}));
      reader.index=page-1;reader.getPageUrl=async()=>fresh.pages[page-1];
      if(page===17)assert.equal(reader.currentPanels.length,6,'six independent page17 panels at each resampling quality');
      // The page7 bottom strip is a known resampling-dependent abstention.
      const targets=labels.frames.filter(f=>f.page===page&&(page!==7||f.name==='red bomber strip'));
      for(const target of targets){
        captures=[];
        ...
        captures[0]._quad.forEach((p,i)=>assert(Math.abs(p.x-target.quad[i][0])<=target.tolerance&&Math.abs(p.y-target.quad[i][1])<=target.tolerance,`Skia ${quality} page${page} follows independent visible borders`));
        probes+=5;
      }
    }
