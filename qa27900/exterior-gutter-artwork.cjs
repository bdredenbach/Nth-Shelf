...
const labels=require('./frame-accuracy/queue-artwork-27917.json');
...
(async()=>{
  let count=0;
  for(const page of [12]){
    const panels=await fresh.api.PanelDetect.detect(fresh.pages[page-1]);
    assert.equal(panels.length,{12:5}[page],`page${page}: independently identified frame count`);
    if(process.env.NTH_BASELINE_ROOT){
      ...
      previous.forEach(p=>assert.ok(panels.some(n=>JSON.stringify(plain(n))===JSON.stringify(plain(p))),
        `page${page}: preserve every existing .16 frame and all its evidence`));
    }
